...
/*   Updated: 2022/07/05 21:06:11 by achansel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	draw_tex(t_everything *all, t_ray *r, t_image wall_img, int x)
{
	...
	while (y_wall <= r->draw_end)
	{
		...
		if (all->info.map[r->map_x][r->map_y] == 'C'
			&& (unsigned int) color != all->info.fog_color)
			put_pixel(&(all->image), x, y_wall, gen_darker_color(color, 10));
		else
...
int	get_thread_id(bool reset)
{
	...
	static pthread_mutex_t	thread_id_mutex;
	int						old;
...
void	draw_img(t_everything *all)
{
	...
	tid = get_thread_id(false);
	memcpy(&new_ray, &(all->ray), sizeof(t_ray));
	x = ((tid - 1) * (all->info.x_size / RENDERER_THREAD_COUNT)) - 1;
	draw_bg(all, x + 1, (all->info.x_size / RENDERER_THREAD_COUNT) * tid);
	while (++x < (all->info.x_size / RENDERER_THREAD_COUNT) * tid)
	{
		set_ray_pos(all, &new_ray, x);
		set_ray_len(&new_ray);
		perform_dda_bonus(all, &new_ray);
		set_projection(&new_ray, &(all->info));
		perform_adequate_drawings(all, &new_ray, x);
	}
	if (tid == RENDERER_THREAD_COUNT - 1)
	{
